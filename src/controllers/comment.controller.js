...
const commentController = {
    ...
    commentInsert: async (req, res) => {
        try {
            ...
            if (!recipeId || !commentText) {
                throw Error(`Field recipe_id, comment_text belum terisi semua`)
            }
            const data = await commentModel.commentInsertData(recipeId, commentText, usersId)
    ...
    commentEdit: async (req, res) => {
        try {
            ...
            const usersId = req.APP_DATA.tokenDecoded.id
            if (!recipeId || !commentText || !usersId) {
                throw Error("Field recipe_id, comment_text, users_id belum terisi semua")
            }
            const data = await commentModel.commentEditData(id, recipeId, commentText, usersId)
            if (data.rowCount === 0) {
                throw Error(`Edit data gagal karena comment id ${id} tidak ditemukan`)
            }
            success(res, data, 'success', `Edit data comment dengan id ${id} berhasil`)
        } catch (err) {
            failed(res, err.message, 'failed', 'error occured')
        }
    },
    commentDelete: async (req, res) => {
        try {
            ...
            success(res, data, 'success', `Delete data comment id ${id} berhasil`)
        } catch (err) {
            failed(res, err.message, 'failed', 'error occured')
        }
    },
    commentByRecipe: async (req, res) => {
        try {
            ...
            if (data.rowCount === 0) {
                throw Error(`Data comment dengan recipe_id ${recipeId} tidak ditemukan`)
            }
            success(res, data.rows, 'success', 'berhasil menampilkan data comment dan recipe')
        } catch (err) {
            failed(res, err.message, 'failed', 'error occured')
        }
    }
