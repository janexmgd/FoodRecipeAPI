...
const userController = {
    ...
    usersEdit: async (req, res) => {
        try {
            ...
            if (!name || !email || !phone) { // validation for input 
                throw Error('Field name,email,phone belum terisi semua')
            }
            const a = await userModel.usersDetailData(id)
            const data = req.files.photo ? await userModel.usersEditData(id, name, email, phone, req.files.photo[0].filename) : await userModel.usersEditData(id, name, email, phone, a.rows[0].photo)
            if (data.rowCount === 0) {
                throw Error(`Edit data users gagal karena users dengan id ${id} tidak ditemukan`)
            }
    ...
    usersDelete: async (req, res) => {
        try {
            const id = req.params.id
            const data = await userModel.usersDeleteData(id)
            if (data.rowCount === 0) {
    ...
    usersResetPass: async (req, res) => {
        try {
            ...
            if (confirmNewPass === newPass) {
                ...
                userModel.usersResetPassData(id, hashNewPass)
                    .then((result) => {
